...
const Method = ({ method }) => {
  const inlineParams = method.params
    ? `${method.params.join(', ')}, ` :
    '';
  let apiLink = `https://api.dpays.io/${method.method}`;
  if (method.params) {
  ...
  return (
    <div className="mb-5">
      ...
      <pre>
        <code>
          {`dpay.api.${changeCase.camelCase(method.method)}(${inlineParams}function(err, result) {
...
const Operation = ({ operation }) => {
  ...
  return (
    <div className="mb-5">
      ...
      <pre>
        <code>
          {`dpay.broadcast.${changeCase.camelCase(operation.operation)}(wif, ${inlineParams}function(err, result) {
  console.log(err, result);
});`}
        </code>
      </pre>
...
const Dpayjs = () =>
  <div className="container my-5">
    <h1><FormattedMessage id="dpayjs" /></h1>
    <p>
      <FormattedMessage
        id="dpayjs_doc"
        values={{
          version: pkg.version,
          source: <a href="https://github.com/dpays/dpayjs"><FormattedMessage id="source_code" /></a>,
