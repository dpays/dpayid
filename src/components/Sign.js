...
export default class Sign extends Component {
  ...
  sign = async (auth) => {
    ...
    if (type === 'tx') {
      ...
      dpay.broadcast[`${changeCase.camelCase(mappedType)}With`](auth.wif, params, (err, result) => {
        if (!err) {
          if (result && (query.cb || query.redirect_uri) && query.auto_return) {
            window.location.href = query.cb || query.redirect_uri;
          } else {
      ...
      <div className="Sign">
        ...
        {step !== 'loading' && <div className="Sign__content">
          <div className="Sign_frame">
            ...
            {step === 'signin' &&
            <div className="Sign__signin-warning"><FormattedMessage id="dpayid_website_confirm" /></div>}
            ...
            <div className="Sign__footer">
              <Link to="/" target="_blank" rel="noopener noreferrer"><FormattedMessage id="about_dpayid" /></Link>
            </div>
          </div>
        </div>}
      </div>
