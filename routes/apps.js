...
router.post('/@:clientId', authenticate('user'), async (req, res) => {
  ...
  const accounts = await req.dpay.api.getAccountsAsync([clientId]);
  if (!accounts[0]) {
    ...
    const requiredAuthStr = JSON.stringify({
      owner: { weight_threshold: 1, account_auths: [['dpayid', 1]], key_auths: [[offlinePubKeys.owner, 1]] },
      active: { weight_threshold: 1, account_auths: [['dpayid', 1]], key_auths: [[offlinePubKeys.active, 1]] },
      posting: { weight_threshold: 1, account_auths: [['dpayid', 1]], key_auths: [[offlinePubKeys.posting, 1]] },
...
router.all('/authorized', authenticate('user'), async (req, res) => {
  const accounts = await req.dpay.api.getAccountsAsync([req.user]);
