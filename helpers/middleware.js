 ...
 * And if app allow @dpayid to post on his behalf
 */
const verifyPermissions = async (req, res, next) => {
  const accounts = await req.dpay.api.getAccountsAsync([req.proxy, req.user]);
  ...
  if (req.proxy) {
    ...
    if (!appStatus || (appStatus && appStatus.is_disabled)) {
      res.status(401).json({
        ...
        error_description: 'This application has been disabled by DPayId',
      });
      return;
    }
  }
