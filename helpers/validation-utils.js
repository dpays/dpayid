...
const allowedSymbols = [
  { symbol: 'BBD', precision: 3 },
  { symbol: 'BEX', precision: 3 },
  { symbol: 'BP', precision: 3 },
...
const userExists = async (username) => {
  ...
  const accounts = await dpay.api.getAccountsAsync([nUsername]);
  return accounts && accounts.length > 0 && accounts.find(a => a.name === nUsername);
};

const contentExists = async (auhtor, permlink) => {
  const content = await dpay.api.getContentAsync(auhtor, permlink);
